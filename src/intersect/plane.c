...
** The function returns a t_intersection struct containing the intersection validity and t value.
** If the ray is parallel to the plane (denominator close to zero), no intersection occurs.
...
** denominator is the dot product of the ray direction and the plane normal, used to determine parallelism.
** If the absolute value of the denominator is less than EPSILON, the ray is considered parallel to the plane.
** If not parallel, the intersection t value is computed and marked as valid.
** The intersection t value represents the distance along the ray to the intersection point.
*/
